...
// Re-export the embedded-can crate so downstream users can access trait /
// identifier types via `canlib::embedded_can::*` without declaring a
// separate dependency. This guarantees they share the same version we
// implement against.
...
// Aliased to avoid colliding with the crate's other "error" terminology;
// the original name is still reachable via `canlib::embedded_can::ErrorKind`.
